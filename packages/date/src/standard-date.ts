import { stdDateMaskMap } from "./mask";
import { useCurrentDate, useStandardDateRegexp } from "./storage";
...
function getDateMap(text: string): IFormatDateMap {
  ...
  const dateList = text.match(useStandardDateRegexp().dateRegexp) || [];

  dateList.forEach((dateText) => {
    // 下面解析顺序是固定的
    const { YYYYMMDDRegexp, YYMMDDRegexp, MMDDRegexp, MMDDYYYYRegexp, MMDDYYRegexp, DDRegexp } =
...
function parseStandardDate(text: string) {
  let result = text;
  ...
  const formatRegexpMap: Record<string, RegExp> = {
  ...
  Object.entries(dateTextMap).forEach(([format, dateTextList]) => {
    ...
    dateTextList.forEach((dateText: string) => {
